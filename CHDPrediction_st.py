...
def main():
    # Title
    st.title('10-year Risk Prediction for Developing Coronary Heart Disease (CHD) Calculator')
    ...
    currentSmoker = st.text_input('Curent smoker (1 = yes, 0 = no):')
    cigsPerDay = st.text_input('Number of cigarettes per day:')
    BPMeds = st.text_input('Takes blood pressure medication(s) (1 = yes, 0 = no):')
    prevalentStroke = st.text_input('History of stroke (1 = yes, 0 = no):')
    prevalentHyp = st.text_input('History of high blood pressure (1 = yes, 0 = no):')
    diabetes = st.text_input('History of diabetes (1 = yes, 0 = no):')
    ...
    if st.button('Click Here for the CHD Prediction'):
        dx = CHD_prediction(
